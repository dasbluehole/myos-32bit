...
//   set_vector(_int1c, 0x1c, (D_INT+D_PRESENT+D_DPL1));
   ...
   kprintf("Initialising Floppy driver...  \n");
	switch(i)
	{
		case 0xFF: kprintf(" [ controller not found ]\n");
				break;
...
#ifdef DEBUG
	kprintf( "clearing disk change status...\n" );
#endif
...
{
#ifdef DEBUG
	kprintf( "recalibrating, motor spin up...\n" );
#endif
...
#ifdef DEBUG
	kprintf( "recalibrating, wait for seek...\n" );
#endif
...
#ifdef DEBUG
	kprintf( "recalibrating, motor off...\n" );
#endif
...
#ifdef DEBUG
	kprintf( "recalibrating, complete...\n" );
#endif
}
...
BOOL fdc_rw(int block,BYTE *blockbuff,BOOL read,unsigned long nosectors)
{
   ...
   for (tries = 0;tries < 3;tries++) {
	 ...
	 recalibrate();
	 motoroff();
	 kprintf("FDC: Disk change detected. Trying again.\n");
	 ...
	 return fdc_rw(block, blockbuff, read, nosectors);
      ...
      if (!flseek(track)) {
	 motoroff();
	 kprintf("FDC: Error seeking to track\n");
	 return FALSE;
      }
